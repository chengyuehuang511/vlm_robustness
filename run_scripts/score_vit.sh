#!/bin/bash
#SBATCH --job-name=calcvit
#SBATCH --output=calcvit.out
#SBATCH --error=calcvit.err
...
concept_list_json='[["uni_image"]]'

srun -u python -m torch.distributed.run --nproc_per_node=1 /nethome/bmaneech3/flash/vlm_robustness/ood_test/contextual_ood/measure.py --ft_method "vit"  --concept_list "$concept_list_json"