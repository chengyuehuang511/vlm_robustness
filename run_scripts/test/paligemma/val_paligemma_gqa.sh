...
srun -u python -m torch.distributed.run --nnodes=1 --nproc_per_node=2 --master_port=25678 evaluate.py --cfg-path configs/paligemma/gqa_val.yaml --options run.init_lr=$init_lr run.min_lr=$min_lr run.warmup_lr=$warmup_lr run.weight_decay=$weight_decay
