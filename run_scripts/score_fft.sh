#!/bin/bash
#SBATCH --job-name=calcfft
#SBATCH --output=calclfft.out
#SBATCH --error=calcfft.err
...
srun -u python -m torch.distributed.run --nproc_per_node=1 /nethome/bmaneech3/flash/vlm_robustness/ood_test/contextual_ood/measure.py --ft_method "fft"  --concept_list "$concept_list_json"