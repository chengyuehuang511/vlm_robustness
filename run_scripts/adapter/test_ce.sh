#!/bin/bash
#SBATCH --job-name=testvqace
#SBATCH --output=testvqace.out
#SBATCH --error=testvqace.err
...
srun -u python -m torch.distributed.run --nproc_per_node=8 evaluate.py --cfg-path /nethome/bmaneech3/flash/vlm_robustness/configs/paligemma/vqa_ce_test.yaml
