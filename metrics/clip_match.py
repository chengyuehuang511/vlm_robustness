...
from metrics.torchmetrics_ext import MetricExt
...
def compute_class_embeddings_given_embedder(
    ...
    if use_cache and cache_file.is_file():
        ...
        logging.debug(f"Loaded class embeddings shape {data.shape} from {cache_file}")
        return data
...
class EmbeddingAccuracy(MetricExt):
    def __init__(
        ...
        **kwargs,
        ...
        self.add_state("pred_keys", default=[])
        self.add_state("pred_values", default=[])
        self.add_state("target_ids", default=[])
    ...
    def update(self, keys: List[str], values: List[str], target_ids: List[int]) -> None:
        ...
        Args:
            keys (list[str]): Batch of keys to identify the datapoint(sample).
    ...
    def compute_logits(
        ...
        return compute_logits(
            target_embeddings,  # gt
            self.embedder,
            self.pred_values,  # preds
...
def main():
    ...
    # ]
    captions = [
        "an image of black cats",
        "an image of a white dog",
        "an image of a black dog",
        "an image of a grouse",
    ]
    metric = EmbeddingAccuracy(class_names=labels)
    metric.update(["0", "1", "2", "3"], captions, [0, 1, 2, 3])
    print(metric.compute_logits())
