...
from data.imagenet_hierarchy import load_hierarchy

from tasks.followup import Followup
from data.classifier_vqa_dataset import ClassifierVQADataset
from tasks.classifier_vqa_task import (
    ClassifierVQATask,
    convert_list_to_dict,
    eval_classifier_vqa,
)
from tasks.vqa_task_utils import save_vqa_output
from utils_folder.result_loader import read_single_result
...
class ClassifierVQAFollowupTask(ClassifierVQATask):
    ...
    def build_datasets(self, cfg):
        ...
        self.followup_prev_dir = followup_prev_dir
    ...
    def _report_metrics(self, result_file, split):
        ...
        # evaluate old answers
        old_evaluate_txt = os.path.join(self.followup_prev_dir, "evaluate.txt")
        if os.path.exists(old_evaluate_txt):
            with open(old_evaluate_txt, "r") as f:
                for line in f:
                    logging.info(line.strip())
        else:
            logging.info(f"Old evaluation file not found: {old_evaluate_txt}")
