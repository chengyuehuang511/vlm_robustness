...
class MinNormSolver:
    ...
    def _projection2simplex(y):
        ...
        """
        m = len(y)
        sorted_y = torch.flip(torch.sort(y), axis=0)
        tmpsum = 0.0
    ...
    def find_min_norm_element(vecs):
        ...
        while iter_count < MinNormSolver.MAX_ITER:
            grad_dir = -1.0*torch.dot(grad_mat, sol_vec)
...
if __name__ == '__main__':
    # Test the min norm solver
    vecs = [torch.randn(10) for _ in range(2)]
    # print(vecs)
    print(vecs[0], vecs[1])
    print(gamma(vecs[0], vecs[1]))
    sol, obj = MinNormSolver.find_min_norm_element(vecs)
    print(sol, obj)
    print('Done')