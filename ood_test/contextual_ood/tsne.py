...
for split in tqdm(splits, desc="Processing splits"): 
    ...
    for i in tqdm(range(len(concept_type)), desc="Processing concept types", leave=False): 
        ...
        plt.title(f"t-SNE plot {concept_type[i]} - {train_ds_split}, {test_ds_split}")

        plt.legend()
        plt.savefig()
        plt.savefig(f"/nethome/bmaneech3/flash/vlm_robustness/result_output/contextual_ood/tsne/annot_{concept_type[i]}_{train_ds_split}_{test_ds_split}.jpg")
        plt.close()
