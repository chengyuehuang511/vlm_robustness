...
for wise in 0
do
    for name in "test_paligemma_vqa_ce" "test_paligemma_vqacp" "test_paligemma_vqa_rep" "test_paligemma_ok-vqa" "test_paligemma_vizwiz" "test_paligemma_textvqa" "test_paligemma_advqa" "test_paligemma_cv-vqa" "test_paligemma_iv-vqa"
    ...
    do
        ...
        output_dir="tpcgrad_share6/output/paligemma/vqa_ft/model_stock_42_0/${job_name}"  # model_stock_42_0  lora_ft_seed_0
        mkdir -p "$output_dir"
        sbatch --export "ALL,wise=${wise},model_stock=${model_stock}" --job-name="${job_name}" --output="${output_dir}/slurm-%j.out" --error="${output_dir}/slurm-%j.err" run_scripts/test/paligemma/${name}.sh
    done
done
